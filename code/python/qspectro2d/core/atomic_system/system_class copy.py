...
class AtomicSystem:
    ...
    def __post_init__(self):
        ...
        self._freqs_cm_history = [self.frequencies_cm.copy()]
    ...
    def lowering_op(self) -> Qobj:
        ...
        if self.max_excitation == 1:
            ...
            for i, mu in enumerate(self.dip_moments, start=1):
                sm_op += mu * (self.basis[0] * self.basis[i].dag())
            return sm_op
        ...
        for (i, j), idx in self._pair_to_index.items():  # i<j
            ...
            sm_op += mu_i * (self.basis[j] * self.basis[idx].dag())
            sm_op += mu_j * (self.basis[i] * self.basis[idx].dag())
        return sm_op
