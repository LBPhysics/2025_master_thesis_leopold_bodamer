...
def _R_paper_1atom(sim_oqs: SimulationModuleOQS) -> Qobj:
    ...
    w0 = sim_oqs.system.at_freqs_fs(0)
...
def _R_paper_2atom(sim_oqs: SimulationModuleOQS) -> Qobj:
    ...
    term = -1j * (sim_oqs.system.omega_ij(1, 0) - omega_laser) - sim_oqs.sb_coupling.Gamma_big_ij(1, 0)
    R[idx_10, idx_10] = term
    R[idx_01, idx_01] = np.conj(term)
    term = -1j * (sim_oqs.system.omega_ij(2, 0) - omega_laser) - sim_oqs.sb_coupling.Gamma_big_ij(2, 0)
    R[idx_20, idx_20] = term
    R[idx_02, idx_02] = np.conj(term)

    # Double-excited coherences
    term = -1j * (sim_oqs.system.omega_ij(3, 0) - 2 * omega_laser) - sim_oqs.sb_coupling.Gamma_big_ij(3, 0)
    ...
    term = -1j * (sim_oqs.system.omega_ij(3, 1) - omega_laser) - sim_oqs.sb_coupling.Gamma_big_ij(3, 1)
    R[idx_31, idx_31] = term
    R[idx_13, idx_13] = np.conj(term)
    term = -1j * (sim_oqs.system.omega_ij(3, 2) - omega_laser) - sim_oqs.sb_coupling.Gamma_big_ij(3, 2)
