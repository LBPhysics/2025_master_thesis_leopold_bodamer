...
The following derivation is part of \autocite{manzanoShortIntroductionLindblad2020}
%----------------------------------------------------------------------------------------
%	SECTION 1
%----------------------------------------------------------------------------------------
...
The following requirements are to be fuildilled by the final derived equation:
\begin{itemize}
...
\end{itemize}
However unlike the Linblad equation it does not guarantee the complete (not even normal) positivity of the density matrix, which is a requirement for a physical state.
So care has to be taken, when the Redfieldequation is useful.
The equation will be valid in the weak coupling limit, meaning that the interaction Hamiltonian $H_I$ is small compared to the system Hamiltonian $H_S$.
...
\end{equation}
This equation will be inserted in Eq. \eqref{eq:Interaction_Picture_Evolution} to obtain the second-order expansion of the density matrix.
...
The step of integration and insertion can be repreated leading to a series expansion of the density matrix:
\begin{equation}
	\dot{\hat{\rho}}_T(t) = -i \alpha \left[ \hat{H}_I(t), \hat{\rho}_T(0) \right]
...
\end{equation}
where third order contributions are neglected.
This can be justified by the fact that the interaction Hamiltonian is small compared to the system Hamiltonian (weak coupling limit).
But remark, that it is still not Morkovian, because of the intergal over time.
...
By taking the interaction Hamiltonian Eq. \eqref{eq:Interaction_Hamiltonian} into account, assuming a seperatability of the system as an initial product state:

\begin{equation}
	\hat{\rho}_T(0) = \hat{\rho}_S(0) \otimes \hat{\rho}_E(0),
	\label{eq:Initial_Product_State}
\end{equation}
and aking the partial trace over the environment, we obtain:
